...
Vendored from ``isaaclab.utils.io.pkl``, which was removed in Isaac Lab 2.3. Upstream now dumps
run configurations as YAML only, but ``env.pkl``/``agent.pkl`` round-trip the config objects
themselves rather than a YAML rendering of them, so they are kept here.
"""
