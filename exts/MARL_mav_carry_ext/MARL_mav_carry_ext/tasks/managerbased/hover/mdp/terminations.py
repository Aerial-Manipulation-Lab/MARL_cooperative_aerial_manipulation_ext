...
def falcon_fly_low(
    ...
    falcon_pos = robot.data.body_com_state_w.torch[:, falcon_idx, :3] - env.scene.env_origins.unsqueeze(1)
...
def falcon_spin(
    ...
    robot = env.scene[asset_cfg.name]
    falcon_idx = robot.find_bodies("Falcon.*base_link")[0]
    falcon_ang_vel = robot.data.body_com_state_w.torch[:, falcon_idx, 10:].reshape(env.scene.num_envs, -1)
...
#     robot = env.scene[asset_cfg.name]
#     falcon_idx = robot.find_bodies("Falcon.*base_link")[0]
#     falcon_quat = robot.data.body_com_state_w.torch[:, falcon_idx, 3:7]
...
def bounding_box(
    ...
    robot = env.scene[asset_cfg.name]
    body_idx = robot.find_bodies(".*")[0]
    body_pos = robot.data.body_com_state_w.torch[:, body_idx, :3]
